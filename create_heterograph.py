...
def test_gaugm(trial):
    ...
    #data
    with open('hypergraph_recon/CoauthorshipCora_probability_upd.pkl', 'rb') as file:
    ...
    for i in range(adj_pred.shape[0]):
        for j in adj_pred.indices[adj_pred.indptr[i]:adj_pred.indptr[i + 1]]:
            edges.append((i, j))
    edge1 = [[], []]
    edge2 = [[], []]
    edge3 = [[], []]
    ...
    for x,y in edges:
        ...
        if x>=n and y>=n:
            ...
            edge4[1].append(y-n)
    g = dgl.heterograph({
        ('type1', 'a', 'type1'): (edge1[0], edge1[1]),
        ('type1', 'b', 'type2'): (edge2[0], edge2[1]),
        ('type2', 'b', 'type1'): (edge3[0], edge3[1]),
        ('type2', 'c', 'type2'): (edge4[0], edge4[1])
    })
    print(g)
    with open('CoauthorshipCora_rgcn.pkl', 'wb') as file:
        pickle.dump(g, file)
    num_type1_nodes = n
    num_type2_nodes = m
    ...
    '''
    # 添加节点
    g.add_nodes(num_type1_nodes, {'type1': range(num_type1_nodes)})
    g.add_nodes(num_type2_nodes, {'type2': range(num_type1_nodes, num_type1_nodes + num_type2_nodes)})

    # 添加边
    # 通过 adj_pred 添加边a（连接类型1的点）
    a_src, a_dst = adj_pred[:num_type1_nodes].nonzero()
    g.add_edges(a_src.tolist(), a_dst.tolist(), etype='a')
    g.add_edges(a_dst.tolist(), a_src.tolist(), etype='a')  # 添加反向边

    # 通过 adj_pred 添加边b（连接类型1和2的边）
    b_src, b_dst = (adj_pred[num_type1_nodes:, :num_type1_nodes] != 0).nonzero()
    g.add_edges(b_src.tolist(), b_dst.tolist(), etype='b')
    g.add_edges(b_dst.tolist(), b_src.tolist(), etype='b')  # 添加反向边

    # 通过 adj_pred 添加边c（连接类型2的点）
    c_src, c_dst = (adj_pred[num_type1_nodes:, num_type1_nodes:] != 0).nonzero()
    c_src = c_src + num_type1_nodes  # 修正节点索引
    c_dst = c_dst + num_type1_nodes  # 修正节点索引
    g.add_edges(c_src.tolist(), c_dst.tolist(), etype='c')
    g.add_edges(c_dst.tolist(), c_src.tolist(), etype='c')  # 添加反向边
...
if __name__ == "__main__":
    ...
    study.optimize(test_gaugm, n_trials=1)
